...
async def upload(
    ...
    return {"message": "Image uploaded successfully", "product_id": product_id}


@app.post("/image/{product_id}", summary="Update product image")
async def update_image(
    ...
    image_bytes = await image.read()
    vector = clip_encoder.encode_image(image_bytes)
    qdrant_service.update_image(product_id, vector, default_status=status)
    return {"message": "Image updated successfully", "product_id": product_id}
...
async def search(image: UploadFile = File(...), threshold: float = 0.7):
    image_bytes = await image.read()
    vector = clip_encoder.encode_image(image_bytes)
    results = qdrant_service.search_similar(vector, score_threshold=threshold)
